...
%>
	...
	<table width=720px align="center">
		<tr>
			<td align="center"><h1 style="font-family: 'Noto Serif KR', serif;">리조트 소식</h1></td>
		</tr>
	</table>
...
<%
	...
	List<Gongji> gongjiList = gongjiService.selectLimit(startId, count); // 전체 공지 리스트 얻기
...
%>
			<td width=50% align="right"><input class="btn btn-default" type="button" value="신규" onclick="location.href='e_01_insert.jsp'"></td>
<%			
		}
%>
		</tr>
...
<nav aria-label="Page navigation" align="center">
  <ul class="pagination">
    <li>
      <a href="http://localhost:8088/resort/e_01.jsp?pageNum=1&count=<%=count%>" aria-label="Previous">
        <span aria-hidden="true">◀◀</span>
        </a>
    </li>
    <li><a href="http://localhost:8088/resort/e_01.jsp?pageNum=<%=nPage%>&count=<%=count%>" aria-label="Previous">
    	<span aria-hidden="true">◀</span>
    	</a>
    </li>
    <%         
    ...
    %>
    <li><a href="http://localhost:8088/resort/e_01.jsp?pageNum=<%=i%>&count=<%=count%>"><%= i %></a></li>
     <%
        }           
        if(endPage < pagecount) {  /* ▶ 에 연결될 페이지 */
    %>
    <li>
      <a href="http://localhost:8088/resort/e_01.jsp?pageNum=<%=startPage+pageblock%>&count=<%=count%>" aria-label="Next">
        <span aria-hidden="true">▶</span>
      </a>
    </li>
    <%      
    ...
    %>
    <li>
      <a href="http://localhost:8088/resort/e_01.jsp?pageNum=<%=pagecount%>&count=<%=count%>" aria-label="Next">
        <span aria-hidden="true">▶▶</span>
      </a>
    </li>
  </ul>
...
%>
	<form method="post" action="e_01_index.jsp" autocomplete="off">
