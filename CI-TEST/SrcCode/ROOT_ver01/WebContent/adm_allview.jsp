...
<body>
	<table border='0' width='883' celpadding='0' cellspacing='0' align=center>
		<tr>
			<td width='250' align='right' valign='middle'><a href="adm_allview.jsp?month=<%=currMonth%>&year=<%=currYear%>&action=0"><font size="5">◀</font></a></td>
			...
			<td width='273' align='left' valign='middle'><a href="adm_allview.jsp?month=<%=currMonth%>&year=<%=currYear%>&action=1"><font size="5">▶</font></a></td>
		</tr>
	</table>
...
<%
			...
			name3 = reservationService.selectReservation(resv_arr[0][order - 1], 3).get(0).getName();
 			if (name1 == null) {
%>
 				 <a href="#" align="center" onclick="submit('<%=resv_arr[0][order - 1]%>', 1)">예약 가능</a><br>
<%
			} else {
%>
				 <a href="#" align="center" onclick="submit('<%=resv_arr[0][order - 1]%>', 1)"><font color="#0000ff"><%=name1%></font></a><br> 
<%
}
 			if (name2 == null) {
%>
 				 <a href="#" onclick="submit('<%=resv_arr[0][order - 1]%>', 2)">예약 가능</a><br>
<%
			} else {
%>
				 <a href="#" onclick="submit('<%=resv_arr[0][order - 1]%>', 2)"><font color="#0000ff"><%=name2%></font></a><br> 
<%
}
			if (name3 == null) {
%>
				<a href="#" color="#ff0000" onclick="submit('<%=resv_arr[0][order - 1]%>', 3)">예약 가능</a><br>
<%
			} else {
%>
				 <a href="#" color="#ff0000" onclick="submit('<%=resv_arr[0][order - 1]%>', 3)"><font color="#0000ff"><%=name3%></font></a><br> 
<%
			}
 		order++;
 		ca2.add(ca2.DATE, +1);
...
<%
 			...
 			if (name1 == null) {
%>
 				<a href="#" color="#ff0000" onclick="submit('<%=resv_arr[0][order2]%>', 1)">예약 가능</a><br>
<%
			} else {
%>
				 <a href="#" onclick="submit('<%=resv_arr[0][order2]%>', 1)"><font color="#0000ff"><%=name1 %></font></a><br> 
<%
}
			if (name2 == null) {
%>
...
%>
				<a href="#" onclick="submit('<%=resv_arr[0][order2]%>', 1)"><font color="#0000ff"><%=name2 %></font></a><br> 
<%						
}
			if (name3 == null) {
%>	
...
%>
				<a href="#"  onclick="submit('<%=resv_arr[0][order2]%>', 1)"><font color="#0000ff"><%=name3 %></font></a><br> 
<%
			}
 				order++;
 				ca.add(ca.DATE, +1);
