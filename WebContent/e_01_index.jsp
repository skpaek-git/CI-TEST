...
%>
	<table width=720px align="center">
		<tr>
			<td align="center" style="font-family: 'Noto Serif KR', serif;"><h1>리조트 소식</h1></td>
		</tr>
	</table>
...
<%
	...
	if("title".equals(field)) {
		List<Gongji> gongjiList = gongjiService.selectTitleSearch(text, startId, count); // 전체 공지 리스트 얻기
...
%>				
		<tr>
			...
			<td width=350 style="background-color:#ffffff"><a href="e_01_view.jsp?id=<%=id%>"><%=title%></a></td>	
<%				
 	if(upload.length() != 0) {
%>	
...
<%
			...
			List<Gongji> gongjiList = gongjiService.selectContentSearch(text, startId, count); // 전체 공지 리스트 얻기
			for(Gongji gongji: gongjiList) {
			int id = gongji.getId();
			String title = gongji.getTitle();
			String date = gongji.getDate();
...
%>				
		<tr>
			<td align="center" style="background-color:#ffffff"><%=id%></td>	
			<td align="center" style="background-color:#ffffff"><%=title %></td>
<% 			
		if(upload.length() != 0) {
%>		
		<td align="center" style="background-color:#ffffff"><img src ="./upload.png" width=20 height=20></td>
...
%>
			<td align="right"><input class="btn btn-default" type="button" value="신규" onclick="location.href='e_01_insert.jsp'"></td>
<%
		}
%>
		</tr>
...
<nav aria-label="Page navigation" align="center">
  <ul class="pagination">
    <li>
      <a href="http://localhost:8088/resort/e_01_index.jsp?pageNum=1&count=<%=count%>&field=<%=field%>&text=<%=text%>" aria-label="Previous">
        <span aria-hidden="true">◀◀</span>
        </a>
    </li>
    <li><a href="http://localhost:8088/resort/e_01_index.jsp?pageNum=<%=nPage%>&count=<%=count%>&field=<%=field%>&text=<%=text%>" aria-label="Previous">
    	<span aria-hidden="true">◀</span>
    	</a>
    </li>
    <%         
    ...
    %>
    <li><a href="http://localhost:8088/resort/e_01_index.jsp?pageNum=<%=i%>&count=<%=count%>&field=<%=field%>&text=<%=text%>"><%= i %></a></li>
     <%
        }           
        if(endPage < pagecount) {  /* ▶ 에 연결될 페이지 */
    %>
    <li>
      <a href="http://localhost:8088/resort/e_01_index.jsp?pageNum=<%=startPage+pageblock%>&count=<%=count%>&field=<%=field%>&text=<%=text%>" aria-label="Next">
        <span aria-hidden="true">▶</span>
      </a>
    </li>
    <%      
    ...
    %>
    <li>
      <a href="http://localhost:8088/resort/e_01_index.jsp?pageNum=<%=pagecount%>&count=<%=count%>&field=<%=field%>&text=<%=text%>">
        <span aria-hidden="true">▶▶</span>
      </a>
    </li>
  </ul>
...
%>
	<form method="post" action="e_01_index.jsp">
