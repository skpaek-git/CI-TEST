...
<%
	...
	GongjiService gongjiService = new GongjiService();  // 객체 생성
...
<Script>
	function submitForm(mode) {
		if (mode == "update") {
			fsf.action = "e_01_update.jsp"; 
		} else if(mode == "delete") {
			fsf.action = "e_01_delete.jsp";
			document.fsf.submit(); // button이므로 submit 해줘야 페이지 넘어감
		}
	}
</Script>
<body link="black" vlink="black" autocomplete="off">
<h1><p align="center" style="font-family: 'Noto Serif KR', serif;">[<%=title%>]</p></h1>
	<form name="fsf" method="post" action="e_01_update.jsp">
...
%>
		...
		<table width=50% align="center">
			<tr>
				<td width=50% align="left"><input class="btn btn-default" type="button" value="목록" onclick="location.href='e_01.jsp'"></td> <!-- 게시판목록으로 이동 -->
<%		
		if(loginOK != null) { // loginOK가 NULL이 아니면(관리자일때)
%>
				<td width=50% align="right"><button class="btn btn-default" type="submit" onclick="submitForm('update')">수정</button><button class="btn btn-default" type="submit" onclick="submitForm('delete')">삭제</button></td>
<%
		}
%>				
			</tr>
